...
G1设计的初衷是为乐尽量缩短处理超大堆时产生的停顿。在回收的时候将对象从一个小堆复制到另一个小堆区，这意味着G1在回收垃圾的时候同时**完成了堆的部分内存压缩**，相对于CMS优势而言就是 **内存碎片的产生率大大降低**。

* Young GC 
  * 堆从一个单一的内存空间划分成了众多的小堆区（reigin），**物理上不分代**。
  * **新生代的内存由一系例不连续的小堆区所组成**。这使得在需要的时候更加容易进行resize。
  * YoungGC是一个**STW事件**，所有应用程序线程都会被暂停。
  * YoungGC使用**多线程并行执行**。
  * 存活的对象会**复制到新的Survivor小堆区**或者**老年代小堆区**。

* Mixed GC
  * 初始标记阶段-Initial Marking Phase（STW）
    * 本阶段和新生代的垃圾回收是密切相关的，另外在本阶段会对GC Roots进行标记。
  * 并发标记阶段-Concurrent Marking Pase
    * 与应用程序并发地查找存货的对象，如果找到了空的小堆区，会在“重新标记阶段”被马上清除。
    * 计算决定活跃度的 “**accounting**“信息
  * 重新标记阶段-Remark Phase（STW）
    * 对于G1，它短暂地停止应用线程，停止并发更新日志的写入，处理其中的少量信息，并标记所有在并发标记阶段开始时未被标记的存活对象。
    * 执行了某些额外的清理，如引用处理或者类卸载。空的小堆区就会被回收
    * 计算所有小堆区的活跃度。
  * 复制/清除阶段-Copying/Cleanup Phase（部分STW）
    * 清除阶段
      * 执行存活对象的accounting完全释放空的小堆区（STW）
      * 擦除RSets（Remembered Sets，作用时跟踪从外部指向本小堆区的所有引用。主要记录从老年代到新生代之间的引用的一个集合，至于新生代之间的应用记录会在每次GC时被扫描，所以不用记录新生代到新生代的引用）（STW）
      * 重置小堆区并将他们归还给free list，也就是空闲表（Concurrent）
    * 复制阶段
      * 本阶段STW 撤离存活对象到新的未被使用的区域。
      * 在新生代小堆区完成时会被记录为[GC pause (young)], 如果在新生代和老年代的小堆区一起执行时会被记录为【GC pause（mixed）】
...
#### 8. 怎么判断内存泄漏？GC调优有经验吗？一般出现GC问题你怎么解决？（淘宝）

#### 9. ThreadLocal有没有内存泄漏？（阿里）

#### 10. G1两个Reign不是连续的，而且之间还有可达的引用，我现在要回收一个，另一个怎么处理？（阿里）

#### 11. 讲一下JVM内存管理机制（对象分配过程）（阿里）

#### 12. 听过CMS的并发预处理和并发可中断预处理吗？（阿里）
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f23531aca890?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f238a8e6c002?imageView2/0/w/1280/h/960/ignore-error/1)
...
调用对象的构造函数，即 Class 文件中的 `()` 来初始化对象，为相关字段赋值。
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f23532cbb5de?imageView2/0/w/1280/h/960/ignore-error/1)

通过直接指针访问对象：

![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f23533e3ce6f?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f238ab08d1eb?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f231a9bd1d61?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f23534c030a8?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f238ab19231c?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f231bfe373b2?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f237ab1d909f?imageView2/0/w/1280/h/960/ignore-error/1)

#### 5.2 ParNew 收集器

他是 Serial 收集器的多线程版本，可以使用多条线程进行垃圾回收：

![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f231c2fcfd60?imageView2/0/w/1280/h/960/ignore-error/1)

#### 5.3 Parallel Scavenge 收集器

Parallel Scavenge 也是新生代收集器，基于 标记-复制 算法进行实现，它的目标是达到一个可控的吞吐量。这里的吞吐量指的是处理器运行用户代码的时间与处理器总消耗时间的比值：

![吞吐量 = \frac{运行用户代码时间}{运行用户代码时间+运行垃圾收集时间}](https://juejin.im/equation?tex=%E5%90%9E%E5%90%90%E9%87%8F%20%3D%20%5Cfrac%7B%E8%BF%90%E8%A1%8C%E7%94%A8%E6%88%B7%E4%BB%A3%E7%A0%81%E6%97%B6%E9%97%B4%7D%7B%E8%BF%90%E8%A1%8C%E7%94%A8%E6%88%B7%E4%BB%A3%E7%A0%81%E6%97%B6%E9%97%B4%2B%E8%BF%90%E8%A1%8C%E5%9E%83%E5%9C%BE%E6%94%B6%E9%9B%86%E6%97%B6%E9%97%B4%7D)

Parallel Scavenge 收集器提供两个参数用于1·-XX:MaxGCPauseMillis：控制最大垃圾收集时间，假设需要回收的垃圾总量不变，那么降低垃圾收集的时间就会导致收集频率变高，所以需要将其设置为合适的值，不能一味减小。
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f231cffbafa6?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f238ab30f205?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f237ac478e93?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f231e73e9f90?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f231e93b9c0c?imageView2/0/w/1280/h/960/ignore-error/1)
...
- `()` 方法与类的构造器函数（即在虚拟机视角中的实例构造器 `()`方法）不同，它不需要显示的调用父类的构造器，Java 虚拟机会保证在子类的 `()` 方法执行前，父类的 `()` 方法已经执行完毕。
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f231ecf573e9?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f235743b552b?imageView2/0/w/1280/h/960/ignore-error/1)
...
![img](https://user-gold-cdn.xitu.io/2020/1/7/16f7f23284a7116e?imageView2/0/w/1280/h/960/ignore-error/1)
...
> **更多文章，欢迎访问 [全栈工程师手册] ，GitHub 地址：[github.com/heibaiying/…](https://github.com/heibaiying/Full-Stack-Notes)**
